...
if (!/^\d+\.\d+\.\d+$/.test(newCoree || '')) {
  console.error(`Usage: propagate-coree-pin.mjs <newCoreeVersion>  (got: ${newCoree})`);
  process.exit(1);
}
...
const record = (cur) => {
  if (oldCoree === null) oldCoree = cur;
    ...
    console.error(`ERROR: inconsistent existing coree pins: ${oldCoree} vs ${cur}`);
    process.exit(1);
  }
};
const PIN_RULES = [
  { re: /(@coree-ai\/coree@)(\d+\.\d+\.\d+)/g, repl: (_m, p1, cur) => { record(cur); return `${p1}${newCoree}`; } },
  { re: /(COREE_VERSION\s*=\s*")(\d+\.\d+\.\d+)(")/g, repl: (_m, p1, cur, p3) => { record(cur); return `${p1}${newCoree}${p3}`; } },
];
...
if (oMaj !== nMaj || oMin !== nMin) {
  ...
  if (existsSync('extension.toml')) {
    ...
    const updated = content.replace(/version\s*=\s*".*"/, `version = "${target}"`);
    if (updated !== content) { writeFileSync('extension.toml', updated); versionChanged = true; console.log(`  version -> ${target}: extension.toml`); }
  }
  pluginVersion = target;
} else {
  console.log(`coree patch bump ${oldCoree} -> ${newCoree}: plugin version unchanged (${pluginVersion})`);
}
