...
Does NOT start coree serve. Instead it manipulates serve.lock and serve.ready
...
def run_inject(binary, cwd):
    """Run coree inject --type session and return (stdout, elapsed_seconds)."""
    t0 = time.monotonic()
...
def main():
    ...
    if sys.platform != "win32":
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                coree_dir = os.path.join(tmpdir, ".coree")
                os.makedirs(coree_dir)

                # mode=local forces serve.lock/serve.ready into .coree/ inside
                ...
                lock_path = os.path.join(coree_dir, "serve.lock")
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            open(os.path.join(coree_dir, "serve.ready"), "w").close()
