...
before(() => {
  ...
  if (sep === '\\') {
    try { execSync('git config core.autocrlf false', { cwd: tmp, stdio: 'pipe' }); } catch {}
  }
});
...
describe('propagate-coree-pin', () => {
  describe('argument validation', () => {
    ...
    it('errors when no tracked files contain a coree pin', () => {
      ...
      writeFileSync(join(dir, 'package.json'), '{"name":"test","version":"0.1.0"}');
  ...
  describe('pin rewriting', () => {
    it('rewrites @coree-ai/coree@ semver pin in npx/hook commands', () => {
      ...
      writeFileSync(join(dir, 'package.json'), '{"name":"test","version":"0.1.0"}');
      writeFileSync(join(dir, 'hook-wrapper.js'), 'npx --yes @coree-ai/coree@0.14.0 inject\n');
      git(dir, 'add package.json hook-wrapper.js');
      ok(dir, ['0.15.0']);
      assert.match(readFileSync(join(dir, 'hook-wrapper.js'), 'utf8'), /@coree-ai\/coree@0\.15\.0/);
    });
    ...
    it('rewrites COREE_VERSION = "..." shell/make pin', () => {
      ...
      ok(dir, ['0.15.0']);
      assert.match(readFileSync(join(dir, 'Makefile'), 'utf8'), /COREE_VERSION\s*=\s*"0\.15\.0"/);
    });

    it('errors on inconsistent pins across different files', () => {
      const dir = freshRepo('pr3');
      writeFileSync(join(dir, 'package.json'), '{"name":"test","version":"0.1.0"}');
      writeFileSync(join(dir, 'hook-a.js'), 'npx --yes @coree-ai/coree@0.14.0 inject');
      writeFileSync(join(dir, 'hook-b.js'), 'npx --yes @coree-ai/coree@0.13.0 inject');
    ...
    it('skips excluded files (CLAUDE.md, AGENTS.md, etc.)', () => {
      ...
      writeFileSync(join(dir, 'package.json'), '{"name":"test","version":"0.1.0"}');
      writeFileSync(join(dir, 'hook.js'), 'npx --yes @coree-ai/coree@0.14.0 inject');
      writeFileSync(join(dir, 'CLAUDE.md'), 'npx --yes @coree-ai/coree@0.13.0\n');
      git(dir, 'add package.json hook.js CLAUDE.md');
      ok(dir, ['0.15.0']);
      assert.match(readFileSync(join(dir, 'hook.js'), 'utf8'), /coree@0\.15\.0/);
      assert.match(readFileSync(join(dir, 'CLAUDE.md'), 'utf8'), /coree@0\.13\.0/);
    });

    it('reports no change when pin already matches', () => {
      const dir = freshRepo('pr5');
      writeFileSync(join(dir, 'package.json'), '{"name":"test","version":"0.1.0"}');
      writeFileSync(join(dir, 'hook.js'), 'npx --yes @coree-ai/coree@0.15.0 inject');
  ...
  describe('plugin version sync', () => {
    it('syncs plugin version to X.Y.0 on major bump', () => {
      ...
      writeFileSync(join(dir, 'package.json'), JSON.stringify({ name: 'test', version: '0.14.2' }));
      writeFileSync(join(dir, 'hook.js'), 'npx --yes @coree-ai/coree@0.14.0 inject');
      git(dir, 'add package.json hook.js');
      ok(dir, ['1.0.0']);
    ...
    it('syncs plugin version to X.Y.0 on minor bump', () => {
      ...
      writeFileSync(join(dir, 'package.json'), JSON.stringify({ name: 'test', version: '0.14.0' }));
      writeFileSync(join(dir, 'hook.js'), 'npx --yes @coree-ai/coree@0.14.0 inject');
      git(dir, 'add package.json hook.js');
      ok(dir, ['0.15.0']);
      const pkg = JSON.parse(readFileSync(join(dir, 'package.json'), 'utf8'));
    ...
    it('syncs .claude-plugin/plugin.json version on major bump', () => {
      ...
      writeFileSync(join(dir, 'package.json'), JSON.stringify({ name: 'test', version: '0.14.2' }));
      writeFileSync(join(dir, 'hook.js'), 'npx --yes @coree-ai/coree@0.14.0 inject');
      writeFileSync(join(dir, '.claude-plugin/plugin.json'), JSON.stringify({ version: '0.14.2' }));
      git(dir, 'add package.json hook.js .claude-plugin/plugin.json');
      ok(dir, ['1.0.0']);
      const plug = JSON.parse(readFileSync(join(dir, '.claude-plugin/plugin.json'), 'utf8'));
